...
if shokey:
    ...
    while run:
        try:
            t_results = shodan.search( search, page=current_page )
            # print(t_results['total'])
            # print(t_results)
        except Exception as e:
            sys.stdout.write( "%s[-] error occurred: %s%s\n" % (fg('red'),e,attr(0)) )
            exit()

        if 'matches' in t_results:
            current_results = len(t_results['matches'])
            total_results = total_results + current_results
            ...
            # yeah i know, it's supposed to be 100!
            if current_results < 90:
                run = False
        else:
            run = False

    if not args.silent:
        sys.stdout.write( '%s[+] %d results found%s\n' % (fg('green'),total_results,attr(0)) )
